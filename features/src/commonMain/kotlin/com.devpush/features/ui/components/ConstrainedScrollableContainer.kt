...
fun ConstrainedScrollableContainer(
    ...
    BoxWithConstraints(modifier = modifier) {
        ...
        // Log constraints in debug builds (simplified version without external logger)
        if (enableLogging) {
            // Simple debug logging - can be enhanced later
            println("ConstrainedScrollableContainer: maxHeight = ${if (maxHeight == androidx.compose.ui.unit.Dp.Infinity) "Infinity" else maxHeight}")
        }
        
        when {
            maxHeight == androidx.compose.ui.unit.Dp.Infinity -> {
                if (enableLogging) {
                    println("⚠️ INFINITE CONSTRAINT DETECTED - applying fallback height: $fallbackHeight")
                }
            ...
            maxHeight == 0.dp -> {
                if (enableLogging) {
                    println("⚠️ ZERO CONSTRAINT DETECTED - applying fallback height: $fallbackHeight")
                }
...
fun FlexibleConstrainedScrollableContainer(
    ...
    BoxWithConstraints(modifier = modifier) {
        ...
        if (enableLogging) {
            println("FlexibleConstrainedScrollableContainer: maxHeight = ${if (maxHeight == androidx.compose.ui.unit.Dp.Infinity) "Infinity" else maxHeight}")
        }
